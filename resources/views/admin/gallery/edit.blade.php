...
<div class="mb-4">
  ...
  <h3 class="fw-bold mt-2">Edit Photo: {{ $photo->title }}</h3>
</div>
    ...
    <form action="{{ route('admin.gallery.update', $photo) }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="row g-3">
        ...
        <div class="col-md-6">
          ...
          @if($photo->image)
            <div class="mb-2">
              <img src="{{ asset('storage/' . $photo->image) }}" alt="Current" class="rounded border" style="max-height: 80px;">
        ...
        <div class="col-md-6">
          ...
          <input type="date" name="event_date" class="form-control" value="{{ old('event_date', $photo->event_date ? $photo->event_date->format('Y-m-d') : '') }}">
        </div>

        <div class="col-12">
          <label class="form-label fw-semibold">Caption / Description</label>
          <textarea name="caption" class="form-control rich-editor" rows="3">{{ old('caption', $photo->caption) }}</textarea>
        </div>
        ...
        <div class="col-12">
          <div class="form-check form-switch mt-2">
            ...
            <label class="form-check-label fw-semibold" for="is_published">Published on website</label>
          </div>
        </div>
